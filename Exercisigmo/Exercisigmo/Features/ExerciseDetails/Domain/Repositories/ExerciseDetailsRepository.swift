//
//  ExerciseDetailsRepository.swift
//  Exercisigmo
//
...
struct ExerciseDetailsRepository: ExerciseVariationsFetcher {
    let dataSource: ExerciseVariationsGetter

    init(dataSource: ExerciseVariationsGetter) {
        self.dataSource = dataSource
    }

    func fetchVariationsExercises(for variationsIDs: [Int]) async -> AnyPublisher<[ExerciseVariationModel], NetworkError> {
        return await dataSource.getExerciseVariationsByIds(for: variationsIDs)
            .map({ entities in
                entities.map { entity in
                    transformExerciseEntityToModel(entity)
                }
            })
            .eraseToAnyPublisher()
    }
...
extension ExerciseDetailsRepository: EntityToModelTranforming {
    ...
    func transformExerciseEntityToModel<T, E>(_ entity: E) -> T where T : Decodable, E : Decodable {
        let entity = entity as! ExerciseVariationsEntity
